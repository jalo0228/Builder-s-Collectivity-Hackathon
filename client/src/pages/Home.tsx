...
export default function Home() {
  ...
  const { data: profiles, isLoading, error } = useSearchProfiles(activeQuery);
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="relative max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <div>
              ...
              <h1 className="text-5xl sm:text-6xl font-extrabold font-display tracking-tight text-foreground mb-4">
                Find the perfect <br />
                <span className="text-gradient">local professional.</span>
              </h1>
              <p className="text-lg text-muted-foreground max-w-xl">
                Describe what you need in plain English. Our semantic search will match you with the right skills and services nearby.
              </p>
            </div>
          ...
          <form onSubmit={handleSearch} className="relative max-w-3xl group">
            ...
            <div className="absolute inset-y-2 right-2 flex items-center">
              <Button type="submit" size="lg" className="rounded-xl px-8 h-full shadow-md font-semibold text-base hover:-translate-y-0.5 transition-transform">
                Search
              </Button>
            </div>
          </form>
      ...
      <main className="flex-1 max-w-[1600px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 h-[calc(100vh-400px)]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full">
          {/* Results List View */}
          <div className={`col-span-1 lg:col-span-5 xl:col-span-4 flex flex-col h-full ${mobileView === 'map' ? 'hidden lg:flex' : 'flex'}`}>
            <div className="flex items-center justify-between mb-4 px-1">
              <h2 className="text-xl font-bold font-display">
                {activeQuery ? `Results for "${activeQuery}"` : "Local Services"}
              </h2>
              <span className="text-sm font-medium bg-secondary text-secondary-foreground px-3 py-1 rounded-full">
                {profiles?.length || 0} found
              </span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pr-2 pb-20 lg:pb-0 space-y-4 custom-scrollbar">
              {isLoading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="h-44 bg-card rounded-2xl animate-pulse border border-border/50" />
                ))
              ) : error ? (
                ...
                <div className="text-center p-12 bg-card rounded-2xl border border-dashed flex flex-col items-center justify-center">
                  ...
                  <p className="text-muted-foreground">Try adjusting your search terms or be the first to list a service!</p>
                </div>
              ) : (
                <AnimatePresence>
          ...
          {/* Map View */}
          <div className={`col-span-1 lg:col-span-7 xl:col-span-8 h-[600px] lg:h-full rounded-3xl overflow-hidden shadow-sm border border-border/60 relative ${mobileView === 'list' ? 'hidden lg:block' : 'block'}`}>
